...
class NewBulkController extends Controller
{
    ...
    public function productUpload(Request $request)
    {
        ...
        foreach ($master as $key => $item) {
            ...
            if ($variation_count != null && $variation_count != '') {
                echo "Have Varient !!".newline(3);
                magicstring($variationType_array);
                
                $CUSTOM_FIELDS = $user->custom_attriute_columns;
                $NUMVBER_CUSTOM_FIELDS = count(json_decode($CUSTOM_FIELDS)) ?? 0;

                echo "NUMBER OF CUSTOM FIELDS ".$NUMVBER_CUSTOM_FIELDS.newline(2);
                
                
                foreach ($variationType_array as $key => $variation) {
                    $tmp_col =  $col_list->{$variation};
                    foreach (explode($Array_saprator,$item[$tmp_col]) as $key => $value) {
                        ...
                        // ` printing Product Obejct
                        magicstring($product_obj);
                            echo $value.newline();
                            $product_att_val = ProductAttributeValue::where('attribute_value',$value)->first();
                ...
                echo "Does not Have Varient";
            }


            echo $count." Products are Created..";
    ...
    public function productUploadOld(Request $request)
    {
        // chk user have active package or not!
        if(AuthRole() == "User"){
            if(!haveActivePackageByUserId(auth()->id())){
