...
class NewBulkController extends Controller
{
    ...
    public function exportDataCustom(Request $request,User $user_id) {
        $this->productBulkExport($request->myfields);
        ...
        foreach ($master as $tmp_key => $tmp_item) {
            ...
            if (isset(${'Model_Code'})) {
                ...
                if (${'Model_Code'} != null && ${'Model_Code'} != "") {
                    // Checking Group Id..
                    $checkSKU = Product::where('model_code',${'Model_Code'})->where('user_id',auth()->id())->first();
                    if (empty($checkSKU)) {
                        // Group Id not Match with User  
                        // Todo: Do Nothing
                    }elseif($checkSKU->user_id != auth()->id()){
                ...
                return back()->with('error',"Model Code Should Require.");
            }
