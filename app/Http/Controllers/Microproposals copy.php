...
class Microproposals extends Controller
{
   ...
   /**
    ...
    function edit(Request $request, Proposal $proposal, $user_key) {   
        ...
        if($proposal->relate_to == $proposal->user_shop_id && $proposal->user_shop_id != $user_shop->id){
            if ($proposal->type != 1) {
                echo "You Don't Have Access of This Page..";
            }
            $ids = ProposalItem::where('proposal_id',$proposal->id)->pluck('product_id');
        ...
        if ($request->ajax()) {
            return view('frontend.micro-site.proposals.load',compact('categories','brands','slug','user_shop','colors','sizes','material','items','user_key','proposalid','excape_items','added_products','proposal','suppliers','request','TandADeliveryPeriod','TandAStock'));
        }else{

            return view('frontend.micro-site.proposals.index',compact('categories','brands','slug','user_shop','colors','sizes','material','items','user_key','proposalid','excape_items','added_products','proposal','suppliers','request','TandADeliveryPeriod','TandAStock'));
