        ...
        <div class="container mt-5 canvas_div_pdf">
            @if ($cust_details['customer_name'] != '' || $proposal->proposal_note != null)
                <div class="row justify-content-center">
                    ...
                    @php
                        $offerbanner = App\Models\Media::where('type_id',$proposal->user_id)->where('type','OfferBanner')->get();
                        if ($offerbanner->count() != 0) {
                            $offerbannerPath = $offerbanner[0]->path;
                        }else{
...
@section('InlineScript')
    ...
    @if ($selectedProp == [] && $selectedProp == null &&  request()->get('download') == null)
        <script>
                $("#jaya1").click(); 
        </script>
    @endif
