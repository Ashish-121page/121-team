    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            // ` Making Array Of shop-image 
                ...
                for (let index = 0; index < passedArray.length; index += 2) {
                    
                    // {{-- ` Set One--}}
                    // ! Adding Up Main Product Details
                    slides[index+1] = pptx.addSlide();
                    ...
                    // rows.push(["Model Code", ":", modelArray[i]]);
                    
                    // rows.push(["Description", ":", descrArray[i]]);

                    // rows.push(["Price", ":", priceArray[i]]);
                    ...
                    if (titleArray != null) {
                        if (titleArray[index + 1] ?? '' != '') {
                            rows.push(["Product TITLE", ":", titleArray[index + 1]]);
                        }
                    }
                    if (modelArray != null) {
                        if (modelArray[index + 1] ?? '' != '') {
                            rows.push(["Model Code", ":", modelArray[index + 1]]);
                        }
                    }
                    if (modelArray != null) {
                        if (modelArray[index + 1] ?? '' != '') {
                            rows.push(["Description", ":", modelArray[index + 1]]);
                        }
                    }
                    
                    if (priceArray != null) {
                        if (priceArray[index +1 ] ?? '' != '') {
                            rows.push(["Price", ":", priceArray[index + 1]]);
                        }
                    }
                ...
                //     // rows.push(["Price", ":", priceArray[i]]);
                    ...
                    if (Additional1Array != null) {
                        if (Additional1Array[index + 1] ?? '' != '') {
                            rows.push([ `${ $(".print_content0").text().split(":")[0].trim()}`, ":" , Additional1Array[index + 1].split(":")[1].trim() ?? '']);    
                        }
                    }
                    if (Additional2Array != null) {
                        if (Additional2Array[index + 1] ?? '' != '') {
                            rows.push([ `${ $(".print_content1").text().split(":")[0].trim()}`, ":" , Additional2Array[index + 1].split(":")[1].trim() ?? '']);
                        }
                    }
                    if (Additional3Array != null) {
                        if (Additional3Array[index + 1] ?? '' != '') {
                            rows.push([ `${ $(".print_content2").text().split(":")[0].trim()}`, ":" , Additional3Array[index + 1].split(":")[1].trim() ?? '']);
                        }
                    }
            ...
            // Put Logo on 3rd Slide Cover Image
            // ` Take Size in Inches Convent Them Using Online converter Cm to Inches
            // lastslide.addImage({
            //     path: logo,
            //     x: '0.25',
            //     y: '0.157',
            ...
            // });
                ...
                // Adding Up Product Info
                // rows = [];
                // rows.push(["Product ID", ":", "2145"]);
                // rows.push(["Model Code", ":", "Model-1235"]);
                // rows.push(["Description", ":", "Dummy Description"]);
                // rows.push(["Price", ":", "170"]);

                // forthpage.addTable(rows, {
                //     x: 5.80,
                //     y: 3.35,
                //     w: 2.91,
                //     h: 2.04
