...
*/
$breadcrumb_arr = [
    ['name'=>'Edit Group User', 'url'=> "javascript:void(0);", 'class' => '']
]
@endphp
                ...
                <div class="col-lg-8">
                    <div class="page-header-title">
                        ...
                        <div class="d-inline">
                            ...
                            <span>Update a record for Group User</span>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
            ...
            <div class="col-md-8 mx-auto">
               ...
               @include('backend.include.message')
                ...
                <div class="card ">
                    ...
                    <div class="card-body">
                        <form action="{{ route('panel.group_users.update',$group_user->id) }}" method="post" enctype="multipart/form-data" id="GroupUserForm">
                            @csrf
                            <input type="hidden" name="group_id" value="{{$group_user->group_id}}">
                            <div class="row">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select required name="group_id" id="group_id" class="form-control select2">
                                            ...
                                            @foreach(App\Models\Group::all()  as $option)
                                                <option value="{{ $option->id }}" {{ $group_user->group_id  ==  $option->id ? 'selected' : ''}}>{{  $option->name ?? ''}}</option> 
                                            @endforeach
                                        </select>
                                    </div>
                                </div> --}}
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select required name="user_id" id="user_id" class="form-control select2">
                                            ...
                                            @foreach(UserList()  as $option)
                                                <option value="{{ $option->id }}" {{ $group_user->user_id  ==  $option->id ? 'selected' : ''}}>{{  $option->name ?? ''}}</option> 
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select required name="user_shop_id" id="user_shop_id" class="form-control select2">
                                            ...
                                            @foreach(App\Models\UserShop::all()  as $option)
                                                <option value="{{ $option->id }}" {{ $group_user->user_shop_id  ==  $option->id ? 'selected' : ''}}>{{  $option->name ?? ''}}</option> 
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
