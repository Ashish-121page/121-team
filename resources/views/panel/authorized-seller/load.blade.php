    ...
    <div class="d-flex justify-content-between mb-2">
        ...
        <div>
            ...
            <ul class="dropdown-menu multi-level" role="menu" aria-labelledby="dropdownMenu">
                ...
                <li class="dropdown-item p-0 col-btn" data-val="col_4"><a href="javascript:void(0);"class="btn btn-sm">Verified</a></li>
                <li class="dropdown-item p-0 col-btn" data-val="col_6"><a href="javascript:void(0);" class="btn btn-sm">Created At</a></li>
            </ul>
        </div>
                        ...
                        <tr>
                            <td class="no-export">
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu multi-level" role="menu" aria-labelledby="dropdownMenu">
                                        <a href="{{ route('panel.brand_user.show', $brand_user->id) }}" title="Show Brand"
                                            class="dropdown-item ">
                                            <li class="p-0">Show</li>
                                        </a>
                                        <a href="{{ route('panel.brand_user.edit', $brand_user->id) }}" title="Edit Brand"
                                            class="dropdown-item ">
                                            <li class="p-0">Edit</li>
                                        </a>
                                        <a href="{{ route('panel.brand_user.delete', $brand_user->id) }}" title="Delete Authorized Seller"
                            ...
                            <td class="text-center no-export">#CR{{ getPrefixZeros($brand_user->id) }}</td>
                            <td class="col_2">{{ fetchFirst('App\User', $brand_user->user_id, 'name', '--') }}</td>
                            {{-- @dump($brand_user) --}}
                            <td class="col_4"><span class="badge badge-{{ getBrandRequestStatus($brand_user->status)['color']}}">{{ getBrandRequestStatus($brand_user->status)['name']}}</span></td>

                            <td class="col_1">@if ($brand_user->is_verified == 1)
            ...
            <label for="">Jump To:
                <select name="page" style="width:60px;height:30px;border: 1px solid #eaeaea;" id="jumpTo">
                    @for ($i = 1; $i <= $brand_users->lastPage(); $i++)
                        <option value="{{ $i }}" {{ $brand_users->currentPage() == $i ? 'selected' : '' }}>
                            {{ $i }}</option>
                    @endfor
                </select>
            </label>
