    ...
    <div class="col-4">
        <div class="card text-center">
            <div class="card-body" style="height: 150px;object-fit: contain; width: 150px;">
                ...
                @endphp
                @if ($filetype == 'image')
                    <img src="{{ asset($path) }}" alt="No Image Available." style="height:100%;width: 100%;"
                        class="rounded">
                @else
                    <img src="https://placehold.co/50x50?text={{ $filetype }}" alt="No Image Available."
        ...
        <nav aria-label="Page navigation example">
            <ul class="pagination">
                ...
                <li class="page-item pageassets">
                    <a class="page-link"
                        href="{{ request()->url() }}?view={{ $view }}&pageassets={{ $previousPage }}"
                        aria-label="Previous">
                ...
                @for ($i = 1; $i <= $paginator->lastpage(); $i++)
                    <li class="page-item pageassets @if ($i == $currentPage) active @endif ">
                        <a class="page-link"
                            href="{{ request()->url() }}?view={{ $view }}&pageassets={{ $i }}">{{$i}}</a>
                    </li>
                @endfor

                <li class="page-item">
                    <a class="page-link pageassets"
                        href="{{ request()->url() }}?view={{ $view }}&pageassets={{ $lastPage }}"
                        aria-label="Next">
...
<script>
    ...
    $(document).ready(function() {
        ...
        $("li.page-item").click(function(e) {
            $('a.page-link').attr("href", "#");
            e.preventDefault();
            let jfbsdfg = $(this).children('a').html().replaceAll(" ", "")
            jfbsdfg.trim();
            console.log(jfbsdfg);
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $(".addingitem").click(function(e) {
                        ...
                        let addedassetsFilename1 = [...new Set(
                        addedassetsFilename)];
