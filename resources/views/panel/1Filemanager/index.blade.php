...
<!-- Bootstrap Modal for duplicate file-->

<div class="modal" id="errorModal" tabindex="-1">
    <div class="modal-dialog" style="width:30%; height:50%; top:50%;">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" style="font-weight:900; color:red">Error</h5>
...
<script>
    ...
    //             var isDuplicate = myDropzone.files.some(function(existingFile) {
    //                 return existingFile.name === file.name;
    //             });

    //             if (isDuplicate) {
    //                 // Show error modal for duplicate file
    //                 showErrorModal(file.name + ' already exists!');
    //                 myDropzone.removeFile(file); // Remove the duplicate file
    //             } else {
    //                 // No duplicate found, proceed to upload
    //                 myDropzone.processFile(file);
    //             }
    //         });

    //         // Function to show the Bootstrap modal with error message
    //         function showErrorModal(message) {
    //             var errorMessage = document.getElementById('errorMessage');
    //             errorMessage.textContent = 'Duplicate file found: ' + message;
    ...
    init: function () {
        this.on("addedfile", function (file) {
            // Check if the file with the same name already exists in Dropzone
            if (this.files.some(function (existingFile) {
                return existingFile.name === file.name;
            })) {
                // Remove the file if it's a duplicate
        ...
        this.on("success", function (file, response) {
            ...
            if (recentFilePaths.length > maxPaths) {
                recentFilePaths = recentFilePaths.slice(-maxPaths);
            }
