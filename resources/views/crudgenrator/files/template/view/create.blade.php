 ...
 */
$breadcrumb_arr = [
    ['name'=>'Add {{ $heading }}', 'url'=> "javascript:void(0);", 'class' => '']
]
{{ $data['atsign'] }}endphp
            ...
            <div class="row align-items-end">
                <div class="col-lg-8">
                    <div class="page-header-title">
                        ...
                        <div class="d-inline">
                            <h5>{{ __('Add')}} {{ $heading }}</h5>
                            <span>{{ __('Create a record for')}} {{ $heading }}</span>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
               ...
               {{$data['atsign']}}include('backend.include.message')
                ...
                <div class="card ">
                    <div class="card-header">
                        <h3>{{ __('Create')}} {{ $heading }}</h3>
                    </div>
                    <div class="card-body">
                        <form action="{{ $data['curlstart'] }} route('{{ $data['dotroutepath'].$data['name']}}.store') }}" method="post" enctype="multipart/form-data" id="{{ $data['model'] }}Form">
                            {{$data['atsign']}}csrf
                            <div class="row">
                            @foreach($data['fields']['name'] as $index => $item)
                                        ...
                                        <select required name="{{ $item }}" id="{{ $item }}" class="form-control select2">
                                            ...
                                            {{$data['atsign']}}endphp
                                                {{$data['atsign']}}foreach(explode(',',$arr) as $option)
                                                    <option value="{{ $data['curlstart'] }}  $option }}" {{ $data['curlstart'] }}  old('{{ $item }}') == $option ? 'Selected' : '' }}>{{ $data['curlstart'] }} $option}}</option> 
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select required name="{{ $item }}" id="{{ $item }}" class="form-control select2">
                                            ...
                                            {{$data['atsign']}}foreach({{ $model_path.$data['fields']['table'][$index] }}::all()  as $option)
                                                <option value="{{ $data['curlstart'] }} $option->id }}" {{ $data['curlstart'] }}  old('{{ $item }}') == $option->id ? 'Selected' : '' }}>{{ $data['curlstart'] }}  $option->name ?? ''}}</option> 
                                            {{ $data['atsign'] }}endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6 col-12"> {{-- Textarea --}}
                                    <div class="form-group">
                                        ...
                                        <textarea @if( $data['fields']['default'][$index] == 1 ) required  @endif class="form-control" name="{{ $item }}" id="{{ $item }}" placeholder="Enter {{ ucwords(str_replace('_',' ',$item))  }}">{{ $data['curlstart'] }} old('{{ $item }}')}}</textarea>
                                    </div>
                                </div>
                                @elseif($data['fields']['input'][$index] == 'decimal')

                                <div class="col-md-6 col-12"> {{-- decimal --}}
                                    <div class="form-group {{ $data['curlstart'] }} $errors->has('{{ $item}}') ? 'has-error' : ''}}">
                                        <label for="{{ $item }}" class="control-label">{{ ucwords(str_replace('_',' ',$item)) }}@if( $data['fields']['default'][$index] == 1 )<span class="text-danger">*</span> @endif</label>
                                        <input @if( $data['fields']['default'][$index] == 1 )required @endif  class="form-control" name="{{ $item }}" type="number" step="any" id="{{ $item }}" value="{{ $data['curlstart'] }}old('{{ $item }}')}}" placeholder="Enter {{ ucwords(str_replace('_',' ',$item))  }}">
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6 col-12"> {{-- checkbox radio --}}
                                    <div class="form-group {{ $data['curlstart'] }} $errors->has('{{ $item}}') ? 'has-error' : ''}}"><br>
                                        <label for="{{ $item }}" class="control-label">{{ ucwords(str_replace('_',' ',$item)) }}@if( $data['fields']['default'][$index] == 1 )<span class="text-danger">*</span> @endif</label>
                                        <input   @if( $data['fields']['default'][$index] == 1 ) required @endif class="js-single switch-input" {{$data['atsign']}}if(old('{{ $item }}'))  @if( $data['fields']['default'][$index] == 4 )checked @endif {{$data['atsign']}}endif name="{{ $item }}" type="{{ $data['fields']['input'][$index] }}" id="{{ $item }}" value="1" >
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6 col-12"> {{-- file/img --}}
                                    <div class="form-group {{ $data['curlstart'] }} $errors->has('{{ $item}}') ? 'has-error' : ''}}">
                                        <label for="{{ $item }}" class="control-label">{{ ucwords(str_replace('_',' ',$item)) }}@if( $data['fields']['default'][$index] == 1 )<span class="text-danger">*</span> @endif</label>
                                        <input @if( $data['fields']['default'][$index] == 1 )required @endif  class="form-control" name="{{ $item }}_file" type="{{ $data['fields']['input'][$index] }}" id="{{ $item }}" value="{{ $data['curlstart'] }}old('{{ $item }}')}}" >
                                        <img id="{{ $item }}_file" class="d-none mt-2" style="border-radius: 10px;width:100px;height:80px;"/>
                                    </div>
                                </div>
                                @else
                                ...
                                <div class="col-md-6 col-12"> {{-- Input --}}
                                    <div class="form-group {{ $data['curlstart'] }} $errors->has('{{ $item}}') ? 'has-error' : ''}}">
                                        <label for="{{ $item }}" class="control-label">{{ucwords(str_replace('_',' ',$item)) }}@if( $data['fields']['default'][$index] == 1 )<span class="text-danger">*</span> @endif</label>
                                        <input @if( $data['fields']['default'][$index] == 1 )required @endif class="form-control" name="{{ $item }}" type="{{ $data['fields']['input'][$index] }}" id="{{ $item }}" value="{{ $data['curlstart'] }}old('{{ $item }}')}}" placeholder="Enter {{ ucwords(str_replace('_',' ',$item))  }}" >
                                    </div>
                                </div>
                                @endif
                            @endforeach
