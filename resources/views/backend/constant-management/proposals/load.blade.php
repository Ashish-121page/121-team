            ...
            <div>
                <label for="">Show
                    <select name="length" style="width:60px;height:30px;border: 1px solid #eaeaea;" id="length">
                        ...
                        <option value="500" {{ $article->perPage() == 500 ? 'selected' : ''}}>500</option>
                    </select>
                    entries
                </label>
            </div>
            ...
            <table id="article_table" class="table">
                <thead>
                    <tr>
                        ...
                        <th  class="col_3">Client Logo</th>
                        <!-- <th  class="col_4">Created At</th> -->
                        <th  class="col_4">Items</th>
                        <th  class="col_5">Views</th>
                    </tr>
                </thead>
                <tbody>
                    @if($article->count() > 0)
                            ...
                            <tr>
                                ...
                                {{-- <td class="no-export">
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu multi-level" role="menu" aria-labelledby="dropdownMenu">
                                            <li class="dropdown-item p-0"><a href="{{ route('panel.constant_management.article.show', $item->id) }}" title="View Article" class="btn btn-sm">Show</a></li>
                                ...
                                @php
                                    $proposa = App\Models\Proposal::whereId($item->proposal_id)->first();
                                @endphp
                                <td class="col_1">{{ $proposal->slug ?? '' }}</td>
                                <td class="col_2">{{ NameById($item->user_id) }}</td>
                                <td class="col_3">
                                    <img src="{{ $proposa->client_logo ?? '' }}" alt=" Client Logo" class="img-fluid border p-2">
                                </td>
                                <!-- <td class="col_4">{{ getFormattedDate($item->created_at) }}</td> -->

                                <td class="col_4">
                                    @php
                                        $count = App\Models\ProposalItem::where('proposal_id',$item->proposal_id)->get();
                                    @endphp
                                    {{ count($count) ?? 0 }}
                                </td>
                                <td class="col_5">
                                    @php
                                        $key = explode('/',$item->shorturl_img);
                                        $url_key = $key[4] ?? '';
                                        $view = App\Models\shorturlvisitor::where('url_key',$url_key)->get();
                                    @endphp
                                    {{ count($view) }}
                                </td>
                            </tr>
