            ...
            <div class="col-md-9 mx-auto">
                <div class="card canvas_div_pdf">
                    <div class="card-header">
                        <h3 class="d-block w-100"> <img class="mb-0 header-logo" src="{{  getFrontendLogo(getSetting('frontend_logo'))}}"  alt="" style="width:120px;">
                            <span class="float-right badge @if($order->payment_status == 1) badge-success @else badge-danger @endif">{{$order->payment_status == 1 ? 'Paid' : 'Unpaid'}}</span>
                            <small class="float-right mr-2 mt-1">{{ __('Date: ')}} {{ $order->created_at->format('d/m/Y') }}</small>
                        </h3>
                    </div>
                    <div class="card-body">
                        <div class="row invoice-info">
                            ...
                            <div class="col-sm-4 invoice-col">
                                ...
                                <b>{{ __('Entity Name:')}}</b> {{ json_decode(App\Models\UserAddress::where('user_id',auth()->id())->first()->details)->entity_name ?? "---"}}<br>
                                <b>{{ __('GST Number:')}}</b> {{ json_decode(App\Models\UserAddress::where('user_id',auth()->id())->first()->details)->gst_number ?? '---'}}<br>
                            </div>
                        </div>
                ...
                <a href="javascript:void(0);" style="position: absolute; bottom: 3rem; left: 2rem;" onclick="window.print()" type="button" class="btn btn-primary no-print">Print</a>
            </div>
        </div>
    </div>
    @push('script')
